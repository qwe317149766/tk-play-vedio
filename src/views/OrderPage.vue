	...
	<div class="app-container">
		...
		<div>
			...
			<div class="content">
				...
				<div class="section">
					...
					<div class="service-grid">
						<ServiceCard
							...
							:service="service"
							:price="service.price"
							:is-selected="selectedService === service.key"
				...
				<!-- 播放服务配置 -->
				<PlayConfig
					v-if="selectedService === 'playVedio'"
					v-model="playConfig"
					:unit-num="currentServiceUnitNum" />

				<!-- 点赞服务配置 -->
				<LikeConfig
					v-if="selectedService === 'likeVedio'"
					v-model="likeConfig" />

				<!-- 评论服务配置 -->
				<CommentConfig
					v-if="selectedService === 'commentVedio'"
					v-model="commentConfig" />

				<!-- 私信服务配置 -->
				<FollowConfig
					v-if="selectedService === 'followVedio'"
					v-model="followConfig" />
				...
				<OrderSummary
					...
					:rendered-services="renderedServices"
					:play-config="playConfig"
					:like-config="likeConfig"
		...
		<!-- 卡密查询模态框 -->
		<a-modal
			v-model:open="showCardQueryModal"
			title="🔐 卡密查询"
			width="800px"
			:footer="null">
			<div class="card-query-modal-content">
				<CardQuery
					ref="cardQueryRef"
					:visible="showCardQueryModal"
					@query="handleCardQuery"
				...
				<div class="section">
					<div class="input-group">
						...
						<div style="display: flex; gap: 10px">
							<a-input
								v-model:value="searchOrderKey"
								placeholder="请输入卡密"
								@keypress.enter="searchOrderById"
								style="flex: 1" />
				...
				<div class="section">
					...
					<div style="max-height: 500px; overflow-y: auto">
						<OrderList
							...
							:child-orders="childOrdersMap"
							:child-loading="childLoadingMap"
							@load-children="handleLoadChildOrders" />
					</div>
				</div>
			</div>
		</a-modal>
		...
		<a-modal v-model:open="showSuccess" :footer="null" :closable="false">
			<div class="success-page">
				...
				<div style="display: flex; gap: 10px; margin-top: 20px">
					...
					<a-button block @click="handleContinueOrder"> 继续下单 </a-button>
				</div>
			</div>
		</a-modal>
	</div>
...
const canSubmit = computed(() => {
	...
	if (isServiceType(service, 'play')) {
		...
		return (
			followConfig.value.targetUsers.length > 0 &&
...
async function fetchOrdersByCard(orderKey) {
	...
	const parentList = Array.isArray(parentRes.data?.list)
		? parentRes.data.list
		: []
	return parentList
}
...
const renderedServices = computed(() => {
	return serviceList.value.map((item, index) =>
		normalizeServiceItem(item, index)
	)
})
...
const currentServiceUnitNum = computed(() => {
	...
	const serviceObj = renderedServices.value.find(
		s => s.key === selectedService.value
	)
...
function normalizeServiceItem(item, index) {
	const fallbackKey =
		DEFAULT_SERVICE_KEYS[index] || `service_${item.id || index}`
	const rawKey = (item.product_type && item.product_type.trim()) || fallbackKey
	const key = resolveServiceKey(rawKey)
	const base =
		SERVICE_TYPES[key] || SERVICE_TYPES[resolveServiceKey(fallbackKey)] || {}
...
async function submitOrder() {
	...
	try {
		...
		if (isServiceType(serviceKey, 'play')) {
			...
			orderData.total_tasks =
				likeConfig.value.videoIds.length * likeConfig.value.likeCountPerVideo
		} else if (isServiceType(serviceKey, 'comment')) {
			orderData.video_ids = commentConfig.value.videoIds
			orderData.comment_count_per_video =
				commentConfig.value.commentCountPerVideo
			orderData.comment_templates = commentConfig.value.commentTemplates
			orderData.add_emoji = commentConfig.value.addEmoji
			orderData.total_tasks =
				commentConfig.value.videoIds.length *
				commentConfig.value.commentCountPerVideo
		} else if (isServiceType(serviceKey, 'follow')) {
...
function showSuccessModal(orderData) {
	...
	successData.value = {
		...
		service:
			...
			SERVICE_TYPES[selectedService.value]?.name ||
			'未知服务',
		quantity: `${orderData.pay_num || 0} 单`,
		pay_amount: orderData.pay_amount
			? `${orderData.pay_amount} 积分`
			: '0.00 积分',
	}
